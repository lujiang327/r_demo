...
cell_cycle_dir <- file.path(figure_dir, "cell_cycle_prpc")
ensure_dirs(output_dir, cell_cycle_dir)

manual_col <- "ManualCelltype"
target_celltype <- "pRPC"
...
plot_stacked_phase <- function(prop_df, path_prefix) {
  ...
  p <- ggplot(plot_df, aes(x = sample_id, y = percent, fill = group)) +
    ...
    labs(x = NULL, y = "pRPC cells (%)", fill = "Phase", title = "pRPC cell cycle phase proportions") +
...
plot_grouped_phase <- function(prop_df, path_prefix) {
  ...
  p <- ggplot(plot_df, aes(x = group, y = percent, fill = sample_id)) +
    ...
    labs(x = "Cell cycle phase", y = "pRPC cells (%)", fill = NULL, title = "pRPC cell cycle phase by sample") +
    theme_classic(base_size = 12) +
...
plot_score_scatter <- function(metadata, path_prefix) {
  ...
  p <- ggplot(plot_df, aes(x = S.Score, y = G2M.Score, color = CellCyclePhase)) +
    ...
    facet_wrap(~Sample) +
    labs(x = "S score", y = "G2M score", color = "Phase", title = "pRPC cell cycle scores") +
    theme_classic(base_size = 12) +
    theme(plot.title = element_text(face = "bold"))
...
plot_prpc_phase_umap <- function(proj, prpc_metadata, path_prefix) {
  ...
  prpc_df <- merge(
    all_df,
    data.frame(
      cell_id = rownames(prpc_metadata),
      Sample = prpc_metadata$Sample,
      CellCyclePhase = prpc_metadata$CellCyclePhase,
      stringsAsFactors = FALSE
    ),
    by = "cell_id"
  )
  prpc_df$CellCyclePhase <- factor(prpc_df$CellCyclePhase, levels = phase_order)

  p <- ggplot() +
    geom_point(data = all_df, aes(x = UMAP_1, y = UMAP_2), color = "#D9D9D9", size = 0.12, alpha = 0.45) +
    geom_point(data = prpc_df, aes(x = UMAP_1, y = UMAP_2, color = CellCyclePhase), size = 0.18, alpha = 0.9) +
    scale_color_manual(values = phase_colors, drop = FALSE) +
    labs(x = "UMAP_1", y = "UMAP_2", color = "Phase", title = "pRPC cell cycle phase on combined UMAP") +
    theme_classic(base_size = 12) +
    theme(
  ...
  p_sample <- ggplot() +
    geom_point(data = all_df, aes(x = UMAP_1, y = UMAP_2), color = "#D9D9D9", size = 0.12, alpha = 0.45) +
    geom_point(data = prpc_df, aes(x = UMAP_1, y = UMAP_2, color = Sample), size = 0.18, alpha = 0.9) +
    scale_color_manual(values = sample_colors, drop = FALSE) +
    labs(x = "UMAP_1", y = "UMAP_2", color = NULL, title = "pRPC cells on combined UMAP by sample") +
    theme_classic(base_size = 12) +
    theme(
      plot.title = element_text(face = "bold"),
      axis.line = element_line(linewidth = 0.3, color = "black"),
...
if (!manual_col %in% colnames(cell_metadata)) {
  stop("Missing `", manual_col, "`. Run script 20 before pRPC cell cycle scoring.")
}

prpc_cells <- rownames(cell_metadata)[cell_metadata[[manual_col]] == target_celltype]
if (length(prpc_cells) == 0) {
...
expr_prpc <- expr[score_genes, prpc_cells, drop = FALSE]
z_expr <- z_score_rows(expr_prpc)
...
prpc_metadata <- cell_metadata[prpc_cells, , drop = FALSE]
prpc_metadata$S.Score <- as.numeric(s_score[rownames(prpc_metadata)])
prpc_metadata$G2M.Score <- as.numeric(g2m_score[rownames(prpc_metadata)])
prpc_metadata$CellCyclePhase <- phase[rownames(prpc_metadata)]
prpc_metadata$CyclingStatus <- cycling_status[rownames(prpc_metadata)]

all_phase <- rep(NA_character_, nrow(cell_metadata))
names(all_phase) <- rownames(cell_metadata)
all_phase[rownames(prpc_metadata)] <- prpc_metadata$CellCyclePhase
all_s_score <- rep(NA_real_, nrow(cell_metadata))
names(all_s_score) <- rownames(cell_metadata)
all_s_score[rownames(prpc_metadata)] <- prpc_metadata$S.Score
all_g2m_score <- rep(NA_real_, nrow(cell_metadata))
names(all_g2m_score) <- rownames(cell_metadata)
all_g2m_score[rownames(prpc_metadata)] <- prpc_metadata$G2M.Score

proj <- addCellColData(
  ArchRProj = proj,
  data = all_phase,
  cells = names(all_phase),
  name = "pRPC_CellCyclePhase",
  force = TRUE
)
proj <- addCellColData(
  ArchRProj = proj,
...
sample_order <- unique(prpc_metadata$Sample)
if (all(c("TH1", "TH2") %in% sample_order)) {
  sample_order <- c("TH1", "TH2")
}
prpc_metadata$Sample <- factor(prpc_metadata$Sample, levels = sample_order)
prpc_metadata$CellCyclePhase <- factor(prpc_metadata$CellCyclePhase, levels = phase_order)

phase_props <- count_proportions(prpc_metadata, "CellCyclePhase", "pRPC_cell_cycle_phase")
phase_comparison <- compare_two_samples(phase_props)
phase_counts <- as.data.frame(table(prpc_metadata$CellCyclePhase, prpc_metadata$Sample), stringsAsFactors = FALSE)
...
write.csv(prpc_metadata, file.path(output_dir, "prpc_cell_cycle_metadata.csv"), row.names = TRUE)
write.csv(phase_counts, file.path(output_dir, "prpc_cell_cycle_phase_counts_by_sample.csv"), row.names = FALSE)
write.csv(phase_props, file.path(output_dir, "prpc_cell_cycle_phase_proportions_by_sample.csv"), row.names = FALSE)
write.csv(phase_comparison, file.path(output_dir, "prpc_cell_cycle_phase_comparison_TH2_vs_TH1.csv"), row.names = FALSE)
write.csv(gene_set_summary, file.path(output_dir, "prpc_cell_cycle_gene_set_summary.csv"), row.names = FALSE)
...
file.copy(
  file.path(output_dir, "Plots", "prpc_cell_cycle_umap.pdf"),
  cell_cycle_dir,
  overwrite = TRUE
)

plot_stacked_phase(phase_props, file.path(cell_cycle_dir, "prpc_cell_cycle_phase_stacked"))
plot_grouped_phase(phase_props, file.path(cell_cycle_dir, "prpc_cell_cycle_phase_by_sample_grouped"))
plot_score_scatter(prpc_metadata, file.path(cell_cycle_dir, "prpc_cell_cycle_score_scatter"))
plot_prpc_phase_umap(proj, prpc_metadata, file.path(cell_cycle_dir, "prpc_cell_cycle_umap_phase"))

saveArchRProject(ArchRProj = proj, outputDirectory = output_dir, load = FALSE)

message("pRPC cell cycle metadata written to: ", file.path(output_dir, "prpc_cell_cycle_metadata.csv"))
message("pRPC cell cycle figures written to: ", cell_cycle_dir)
